...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 flex justify-center items-center px-4 py-3 bg-white">
      {/* Dark pill container */}
      <div className="bg-[#1a1a1a] rounded-full px-5 py-2 flex items-center w-full max-w-2xl shadow-lg">

        {/* Logo inside pill */}
        <Link href="/" className="flex items-center shrink-0 mr-6" data-testid="link-logo">
          <img
            src={logoDark}
            alt="Monetium"
        ...
        <nav className="hidden lg:flex items-center gap-6 flex-1 justify-center">
          {navItems.map((item) => (
            <a
              ...
              className="text-sm font-medium text-white hover:text-white/75 transition-colors whitespace-nowrap"
            >
              {item.label}
            </a>
          ))}
        ...
        <a
          ...
          className="ml-auto shrink-0 px-4 py-1.5 bg-white text-foreground text-sm font-semibold rounded-full hover:bg-white/90 transition-colors hidden lg:flex items-center"
        ...
        <button
          className="lg:hidden ml-auto p-1 text-white hover:text-white/75 transition-colors"
